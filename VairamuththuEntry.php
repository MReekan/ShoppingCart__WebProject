...
if (isset($_POST['submit'])) {
    ...
    $target = "vairamuththu/" . basename($_FILES['image']['name']);
   ...
   include("db.php");
    ...
    if ($mysqli->query("insert into vairamuththu (name, image,author_name,price,quantity) VALUES ('$name','$image','$author_name'
      ,'$price',' $quantity')")) {
...
<div id="upload">
<div id="add"> <h2 >Entry Vairamuththu's Books</h2></div><br>
<div id="name">
</br></div>
<div id="name">
...
<div>
   ...
   <?php
    include("db.php");
    $sql="SELECT * FROM vairamuththu";
    $result=$mysqli->query($sql);
    while ($row=mysqli_fetch_array($result)) {
              ?>
                 <div style="border: 1px solid black">
                <img src="<?php echo 'vairamuththu/'.$row['image']?>"> <br>
                <?PHP
                        echo  $row['name'] ;
                    $name=$row['name'];
                    echo "&nbsp &nbsp"; 
                    ...
                    <div>
                      ...
                      <a href="VairamuththuBookDetail.php?name=<?php echo $name ?>">ClickMe</a>
                    </div>
